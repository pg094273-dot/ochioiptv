        ...
        try {
            ...
            setTimeout(() => {
                this.video.play().catch(err => {
                    logger.warning('Auto-play bloqueado por navegador');
                });
            }, 500);
    ...
    tryLoad(url) {
        return new Promise((resolve, reject) => {
            ...
            if ((isIOS || isSafari) && this.video.canPlayType('application/vnd.apple.mpegurl')) {
                logger.info('Modo: HLS nativo (iOS/Safari)');
                ...
                this.hls = new Hls({
                    debug: false,
                    enableWorker: true,
                    // SOLUCIÓN PARA EL PROBLEMA DE 1:20 min
                    maxBufferLength: 60,          // Buffer máximo 60s
                    maxMaxBufferLength: 120,      // Buffer máximo absoluto 120s
                    maxBufferSize: 60 * 1000 * 1000, // 60 MB
                    maxBufferHole: 0.5,           // Tolerancia a huecos
...
const player = new IPTVPlayer(document.getElementById('videoPlayer'));